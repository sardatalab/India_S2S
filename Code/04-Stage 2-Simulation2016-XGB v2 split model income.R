...
# Different XGB models: one predicting income for HHs with inc variables and
# another predicting welfare for the rest, Euclidean distance, geom_mean ensemble
...
lfs.don$logwelfare=log(lfs.don$welfare+1)
lfs.don$district=as.factor(lfs.don$district)
lfs.don$loginc=log(lfs.don$rpcinc_tot+1)
...
# Set up parallel back-end using available cores
n_cores <- parallel::detectCores() - 1  # Reserve one core for OS
cl <- makeCluster(n_cores)
registerDoParallel(cl)
...
foreach (a = c(0,1)) %do% {  # 0: HHs with income information, 1 otherwise
    ...
    foreach(sim = 1:n_sim) %do% {
        ...
        if(a==0) {
           mod.a=lm(loginc~.,data=train_sample[, c("loginc",covariates)])
           X_train = model.matrix(mod.a)
           y_train <- train_sample$loginc
        } else {
