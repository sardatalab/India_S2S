...
foreach (a = c(0,1)) %do% {  # 0: HHs with income information, 1 otherwise
  ...
  hh_vars = c("hhsize","hhsize_sq","age_avg","avg_age_sq","share_dep",
              "share_kids","sh_mem_014",
              "sh_mem_1564","sh_mem_male","has_in_school","sh_in_school")
  hhh_vars = c("buddhist_hhh","married_hhh","sinhala_hhh","female_hhh","age_hhh", 
               "edu_hhh_none","edu_hhh_prim","edu_hhh_sec","edu_hhh_secincomp",
               "edu_hhh_high")
  mainind_vars = grep("^hh_main", names(data.don), value = TRUE)
  econ_vars = c("have_agri_emp","have_constr_emp", "have_ind_emp","have_public_emp",
                "have_semiskilled_worker", "sh_selfempl","public_emp_hhh")
  ...
  inc_vars    = c("sh_wages","sh_selfemp")
  #aware_vars = grep("^hh_aware", names(data.don), value = TRUE)
  #can_vars = grep("^hh_can", names(data.don), value = TRUE)
  #used_vars = grep("^hh_used", names(data.don), value = TRUE)
  #internet_vars = grep("^hh_internet", names(data.don), value = TRUE)
  oth_vars    = c("urban","district")
  
  #Combine the covariate names
  if(a == 0) {
      covariates <- c(hh_vars,hhh_vars,econ_vars, 
                      inc_vars,oth_vars,"hidseq")
  ...
  # Create the formula 
  if(a==0){
    formula.mod.a <- as.formula(paste(paste("rpcinc1 ~", 
