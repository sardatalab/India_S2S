...
# This model includes ymatch, prcinc_tot, hhsize, and hhb_year to find nearest
# neighbor
# Second model: Trains model to predict welfare via PMM between 2016
# (donor, trained on HHs with income) and 2016 (receiver, HHS w/o income)
...
X.mtc2=c("ymatch","rpcinc_tot","hhsize","hhb_year")
...
foreach(sim = 1:n_sim) %do% {
    ...
    if (min(table(lfs.don[lfs.don$flag6_income2==0,]$district,
                  lfs.don[lfs.don$flag6_income2==0,]$urban))>0){
...
lfs.don=lfs.imp.0 #Donor dataset now is LFS 2016 based on imputed welfare
mod.full=lm(logwelfare~.,
            data=lfs.don[lfs.don$flag6_income2==0, 
                         c("logwelfare",covariates)])
X_train_full = model.matrix(mod.full)
...
# Parallel grid search using foreach
tuning_results_y2_0 <- foreach(i = 1:nrow(param_grid), 
        .combine = rbind, 
        .packages = "xgboost",
        .export = c("X_train_full", "y_train_full")) %dopar% {
...
# Stop the cluster after tuning
stopCluster(cl)
print(tuning_results_y2_0)
rm(mod.full,X_train_full,y_train_full)

write.csv(tuning_results_y2_0,file=paste(path,
                                      "/Outputs/Intermediate/Models/XGB_tuning_2016_0_v4","_",
                                      Sys.Date(),".csv",sep=""),
          row.names = FALSE)

# Run these lines to load tuning results previously saved
# tuning_results_y2_0=read.csv(paste(path,
#                               "/Outputs/Intermediate/Models/XGB_tuning_2016_0_v4","_",
#                               "2025-11-05",".csv",sep=""))

best_params_row_y2_0 <- tuning_results_y2_0[which.min(tuning_results_y2_0$best_rmse), ]

best_params_y2_0 <- list(
    objective = "reg:squarederror",
    eval_metric = "rmse",
    eta = 0.1,
    max_depth = best_params_row_y2_0$max_depth,
    gamma = best_params_row_y2_0$gamma,
    subsample = best_params_row_y2_0$subsample,
    colsample_bytree = best_params_row_y2_0$colsample_bytree
)
best_nrounds_full_y2_0 <- best_params_row_y2_0$best_iteration

cat("Best hyperparameters from tuning for HHs with income:\n")
print(best_params_y2_0)
...
df.match.1$welfare_median=apply(df.match.1[,-1],
                              1,median,na.rm=TRUE)
df.match.1$welfare_geom=apply(df.match.1[,-1],
                              1,geometric_mean,na.rm=TRUE)
df.match.1$welfare_mean=apply(df.match.1[,-1],
                              1,mean,na.rm=TRUE)
