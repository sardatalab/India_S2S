...
lfs19=lfs.don
lfs19$hhid=NULL
lfs19=subset(lfs19,select=c(district, urban,popwt,welfare, rpcinc_tot))
lfs19$survey="LFS_19"
#2016
lfs16.orig=read_dta(paste(datapath,
                       "lfs2016_imputed.dta",
                       sep="")) 
lfs16.orig=subset(lfs16.orig,select=c(district, urban,popwt,welfare, rpcinc_tot))
...
lfs23.orig=read_dta(paste(datapath,
                          "lfs2023_imputed.dta",
                          sep="")) 
lfs23.orig=subset(lfs23.orig,select=c(district, urban,popwt,welfare, rpcinc_tot))
...
ggplot(lfs.all, aes(x = log(rpcinc_tot), weight = popwt,
               color = survey)) +
...
median(lfs.all[lfs.all$survey=="LFS_23",]$welfare,na.rm=TRUE)/median(lfs.all[lfs.all$survey=="LFS_19",]$welfare,na.rm=TRUE)
