...
# Single XGB model for all observations, Euclidean distance, geom_mean ensemble
# Best results so far as of 11/4/25 9:32 am
...
# -----------------------------
# Step 1: Hyperparameter Tuning on Full 2019 LFS data
# -----------------------------
...
tuning_results <- foreach(i = 1:nrow(param_grid), 
    .combine = rbind, 
    .packages = "xgboost",
...
best_nrounds_full <- best_params_row$best_iteration

cat("Best hyperparameters from tuning:\n")
print(best_params)
cat("Best nrounds (from tuning):", best_nrounds_full, "\n\n")

# -----------------------------
# Step 2: Predictions and PMM
# -----------------------------
...
#match
simcons_match=subset(lfs.rec,sel=c(hhid))
#prediction
simcons_pred=subset(lfs.rec,sel=c(hhid))

# Parallel simulation loop using foreach
# sim_results <- foreach(sim = 1:n_sim,
#             .packages = c("xgboost","dplyr","data.table",
#                           "StatMatch")) %dopar% {
#                             
...
df.match=simcons_match
df.pred=simcons_pred
...
# 
# saveRDS(sim_results,file=paste(path,
#                                "/Results/PLFS matching/XGBoost_sim_results_","_",
#                                Sys.Date(),".rds",sep=""))
# 
# # Aggregate predictions across simulations
# for (year in names(survey_list)) {
#   # Extract predictions for the current year from each simulation and combine them into a matrix
#   year_preds <- sapply(sim_results, function(res) res[[year]])
#   predictions[[year]] <- year_preds
# }
# 
# saveRDS(predictions,file=paste(path,
#                "/Results/PLFS matching/XGBoost_predictions_","_",
#                   Sys.Date(),".rds",sep=""))
# 
# 
# 
